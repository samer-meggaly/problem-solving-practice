...
public class WordLadder {
	...
	public List<List<String>> findLadders(String beginWord, String endWord,
		...
		/*
		 * The following map provides a Key optimization to the normal BFS
		 * traversal of all the possible paths. The optimization is based upon
		 * the fact that the first time the algorithm arrives at a word in the
		 * dictionary, then this will happened with the fewest number of steps.
		 * Any subsequent path that will arrive at the same word will only be
		 * expanded iff it has the same number of steps as the limit discovered
		 * before.
		 */
		Map<String, Integer> shortestStepsToWord = new HashMap<String, Integer>();
		int shortest = Integer.MAX_VALUE;
		for (String s : wordList)
			shortestStepsToWord.put(s, Integer.MAX_VALUE);

		queue.offer(new Path(beginWord, new ArrayList<String>()));
		while (!queue.isEmpty()) {
			Path curr = queue.poll();
			if (curr.pathSteps.size() >= shortest)
				continue;
			char[] lastStep = curr.pathSteps.get(curr.pathSteps.size() - 1)
					.toCharArray();
			for (int i = 0; i < lastStep.length; i++) {
				char oldC = lastStep[i];
				for (int c = 'a'; c <= 'z'; c++) {
					lastStep[i] = (char) c;
					String nextStep = new String(lastStep);
					if (nextStep.equals(endWord)) {
						List<String> outputPath = new ArrayList<String>(
								curr.pathSteps);
						outputPath.add(nextStep);
						results.add(outputPath);
						if (outputPath.size() < shortest)
							shortest = outputPath.size();
					} else if (curr.pathSteps.size() < shortest
							&& shortestStepsToWord.containsKey(nextStep)
							&& curr.pathSteps.size() <= shortestStepsToWord
									.get(nextStep)) {
						Path newPath = new Path(nextStep, curr.pathSteps);
						queue.offer(newPath);
						shortestStepsToWord.put(nextStep,
								curr.pathSteps.size());
					}
				}
				lastStep[i] = oldC;
	...
	private static class Path {
		...
		public Path(String newStep, List<String> prevSteps) {
			pathSteps = new ArrayList<String>(prevSteps);
			pathSteps.add(newStep);
		}
	}
	...
	public static void main(String[] args) {
		...
		beginWord = "cet";
		endWord = "ism";
		wordList = new String[] { "kid", "tag", "pup", "ail", "tun", "woo",
