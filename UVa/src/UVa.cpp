...
template<typename T>
std::string vec2str(std::vector<T> vi, std::string delim = std::string(", ")) {
	std::ostringstream oss;
	oss << "[ ";
	typename std::vector<T>::iterator itr;
	for (itr = vi.begin(); itr != vi.end(); itr++)
		oss << (*itr) << ((itr < vi.end()-1)? delim:" ");
	oss << "]";
	return oss.str();
}
std::vector<int> countSwaps(std::vector<int> list, ull& swaps) {
	if (list.size() == 1) {
		swaps = 0L;
		return list;
	} else {
		...
		left = countSwaps(left, leftSwaps);
		right = countSwaps(right, rightSwaps);
...
ull countMinimumMoves(std::vector<int> list) {
	if (list.size() <= 1) {
		return 0L;
	} else {
		ull swaps = 0L;
		countSwaps(list, swaps);
		return swaps;
	}
}
...
int main() {
	...
	while (cin >> n && n > 0) {
		...
		REP_INC(i,0,n,1)
			cin >> list[i];
		ull moves = countMinimumMoves(list);
		cout << ((moves % 2 == 1)? "Marcelo":"Carlos") << endl;
	}
	return EXIT_SUCCESS;
}
